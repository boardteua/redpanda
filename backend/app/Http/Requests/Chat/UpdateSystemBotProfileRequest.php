...
class UpdateSystemBotProfileRequest extends FormRequest
{
    public function authorize(): bool
    {
        $user = $this->user();

        return $user !== null && $user->can('chat-admin');
    }
    ...
    {
        $botId = User::query()->where('is_system_bot', true)->orderBy('id')->value('id');
        ...
        if ($botId !== null) {
            $userNameRules[] = Rule::unique('users', 'user_name')->ignore((int) $botId);
        } else {
            $userNameRules[] = Rule::unique('users', 'user_name');
        }
