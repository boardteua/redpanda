...
3. **Dev ↔ QA** — **строго одна задача** з task list: implement → **код-рев’ю** (роль [code-reviewer](../../.cursor/skills/code-reviewer/SKILL.md): коректність, безпека, тести) → збір доказів QA → **PASS** / **FAIL** (до 3 спроб на задачу).
