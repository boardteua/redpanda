...
## Усунення регресій (dev vs production)

- **Консоль: `Unknown custom element: <UserAvatar>`** — компонент має бути в `Vue.component(...)` (глобально в `resources/js/app.js`) або в `components` батьківського SFC; одного `import` недостатньо.
- **Порожні квадрати після `vite build`** — див. scoped-стилі в `UserAvatar.vue` (`rp-user-avatar--sm/md`, колір гліфа); переконайтесь, що задеплоєно свіжий `public/build`.
