...
**Епік:** T160 (PWA)  
**Статус:** baseline збережено для порівняння; **T162** реалізовано — див. [T162-QA.md](./T162-QA.md) та §3.1 (серія B після).  
...
### 3.1 Серія B — після реалізації **T162** (очікуваний критичний шлях)

| Сценарій | Мінімум послідовних RTT на критичному шляху | Примітка |
|----------|---------------------------------------------|----------|
| R1 кімната | **1** (лише POST, якщо вже є cookie `XSRF-TOKEN`; інакше **+1** GET `/sanctum/csrf-cookie`) | Умовний `ensureSanctum`, один retry на **419** у `bootstrap.js` |
| R1 (перцепція) | — | Optimistic рядок у стрічці до відповіді API |
| P1 приват | **1–2** (POST; за відсутності XSRF — +1 GET CSRF) | `loadConversations()` після успішного POST **не** блокує UI; optimistic у панелі |
| R2 кімната + новий файл | **2+** (умовний CSRF + `POST /api/v1/images` + POST повідомлення) | Optimistic для вкладень не додано (залишено в P1 за T161) |
